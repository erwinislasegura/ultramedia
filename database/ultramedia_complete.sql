...
CREATE TABLE photo_sets(id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,event_id INT UNSIGNED NOT NULL,name VARCHAR(180) NOT NULL,bib_number VARCHAR(30),cover_photo_id BIGINT UNSIGNED NULL,individual_enabled TINYINT(1) NOT NULL DEFAULT 1,set_enabled TINYINT(1) NOT NULL DEFAULT 1,pack_enabled TINYINT(1) NOT NULL DEFAULT 0,pack_quantity SMALLINT UNSIGNED NOT NULL DEFAULT 5,pack_price INT UNSIGNED NOT NULL DEFAULT 14990,set_price INT UNSIGNED NOT NULL DEFAULT 19990,status ENUM('active','hidden') NOT NULL DEFAULT 'active',created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,INDEX(event_id),INDEX(cover_photo_id),CONSTRAINT fk_set_event FOREIGN KEY(event_id) REFERENCES events(id)) ENGINE=InnoDB;
