...
final class Photo{
 ...
 private static function publicWhere():string{return "p.status='active' AND e.status='published' AND (ps.id IS NULL OR (ps.status='active' AND (ps.individual_enabled=1 OR ps.set_enabled=1 OR ps.pack_enabled=1)))";}
