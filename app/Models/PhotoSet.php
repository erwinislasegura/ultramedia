...
final class PhotoSet{
 public static function find(int $id):?array{$s=Database::db()->prepare("SELECT ps.*,e.name event_name,COUNT(p.id) photos_count,MIN(p.id) cover_id,MIN(p.preview_path) preview_path FROM photo_sets ps JOIN events e ON e.id=ps.event_id JOIN photos p ON p.set_id=ps.id AND p.status='active' WHERE ps.id=? AND ps.status='active' AND e.status='published' GROUP BY ps.id");$s->execute([$id]);return $s->fetch()?:null;}
